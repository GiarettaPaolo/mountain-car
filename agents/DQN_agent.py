...
class DQNAgent(Agent):
    def __init__(self, action_space, Q, optimizer, *,
        ...
        '''
        params:
        Q: torch.nn.Module, the Q-network
        optimizer: torch.optim.Optimizer, the optimizer for the Q-network
        gamma: float, discount factor
        eps: float, epsilon-greedy parameter
        buffer_size: int, size of the replay buffer
        batch_size: int, size of the batch sampled from the buffer
        device: torch.device, device to store the Q-network
        '''
...
class Buffer:
    ...
    device: torch.device, device to store the buffer
    '''
